...
try
{
      ...
      else if($ownership['item_type'] == 'armor')
      {
            ...
            $item = $stmt -> fetch();
            $stmt -> closeCursor();
            if($_SESSION['hero']['constitution'] >= $item['constitution_requirements']) $_SESSION['hero']['armor'] = $_GET['id'];
            else
            {
      ...
      else if($ownership['item_type'] == 'shield')
      {
            ...
            $item = $stmt -> fetch();
            $stmt -> closeCursor();
            if($_SESSION['hero']['dexterity'] >= $item['dexterity_requirements']) if($_SESSION['hero']['left_busy'] == 0) $_SESSION['hero']['hand_left'] = $_GET['id'];
            else
            {
      ...
      else if($ownership['item_type'] == 'weapon_melee')
      {
	      $stmt = $pdo -> query("SELECT strength_requirements, dexterity_requirements, handedness FROM item_weapon_melee WHERE id = ".$ownership['item_id']);
            ...
            {
                  $_SESSION['hero']['hand_right'] = $_GET['id'];
      ...
      else if($ownership['item_type'] == 'tool') $_SESSION['hero']['tool'] = $_GET['id'];
      else if($ownership['item_type'] == 'ammo') $_SESSION['hero']['ammo'] = $_GET['id'];
}
